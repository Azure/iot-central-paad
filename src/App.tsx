    ...
    return (<NavigationContainer theme={mode === ThemeMode.DARK ? DarkTheme : DefaultTheme}>
        <Stack.Navigator>
            <Stack.Screen name='root' options={({ navigation }) => ({
                ...
                {() => (<Tab.Navigator key='tab' screenOptions={({ route }) => ({
...
function Profile(navigate: any) {
    const { dark, colors } = useTheme();
    return (<Icon name={Platform.select({ ios: 'person-circle', android: 'person' })} type={Platform.select({ ios: 'ionicon', android: 'material' })} color={colors.text} onPress={() => {
        navigate('Settings');
    }} />);
}
