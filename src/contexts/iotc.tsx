...
const healthMap: {
  [id in valueof<typeof AVAILABLE_HEALTH>]: Constructor<ISensor>;
} = {
...
export type IIoTCContext = ICentralState & {
  connect: (
    credentials: IoTCCredentials | null | undefined,
    cancellationToken?: CancellationToken,
  ) => Promise<void>;
  disconnect: () => Promise<void>;
  updateSensors: (
    type: 'telemetry' | 'health',
    fn: (currentData: SensorProps[]) => SensorProps[],
  ) => void;
...
const IoTCProvider: React.FC<{children: React.ReactNode}> = ({children}) => {
  ...
  const [state, setState] = useState<ICentralState>({
    client: undefined,
  ...
  const updateValues = (id: string, value: any) => {
    setState((current) => ({
      ...current,
      telemetryData: current.telemetryData.map(({...sensor}) => {
        if (sensor.id === id) {
          sensor = {...sensor, value};
        }
        return sensor;
  ...
  useEffect(() => {
    Object.values(sensorMap).forEach((sensor) => {
      ...
      sensor.addListener(SENSOR_UNAVAILABLE_EVENT, (sensorId: string) => {
        setState((current) => {
          const index = current.telemetryData.findIndex(
            (s) => s.id === sensorId,
          );
          if (index > -1) {
  ...
  const contextObj = {
    ...
    updateSensors: (
      ...
      switch (type) {
        case 'telemetry':
          setState((current) => ({
            ...current,
            telemetryData: fn(current.telemetryData),
          }));
          break;
        case 'health':
          setState((current) => ({
            ...current,
            healthData: fn(
              current.healthData.map(({...h}) => h.sensorProps),
            ).map<IHealthData>(({...sensorProps}) => ({
              sensorProps,
              sensor: (current.healthData.find(
                (h) => h.sensorProps.id === sensorProps.id,
              ) as IHealthData).sensor,
            })),
          }));
      }
    },
    getSensorName: (id: string) =>
      ({
        ...sensorMap,
        ...state.healthData.reduce<{[x: string]: ISensor}>(
          (obj, item) => ({
            ...obj,
            [item.sensor.id as keyof typeof AVAILABLE_HEALTH]: item.sensor,
          }),
          {},
        ),
      }[id].name),
    addListener: (eventname: string, listener: (...args: any[]) => void) => {
      if (eventname === LOG_DATA) {
        ...
        Object.values(sensorMap).forEach((s) =>
          s ? s.addListener(eventname, listener) : null,
        );
    ...
    removeListener: (eventname: string, listener: (...args: any[]) => void) => {
      ...
      Object.values(sensorMap).forEach((s) =>
        s ? s.removeListener(eventname, listener) : null,
      );
      state.healthData.forEach((health) => {
    ...
    initHealth: () => {
      ...
      setState((current) => ({
        ...
        healthData: [
          ...current.healthData,
          ...defaultHealths.map<IHealthData>((health) => {
