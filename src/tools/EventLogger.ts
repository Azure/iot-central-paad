...
import { StateUpdater, TimedLog } from "../types";

export default class EventLogger extends EventEmitter implements IIoTCLogger {

    constructor(private eventName: string, private logLevel: IOTC_LOGGING = IOTC_LOGGING.API_ONLY) {
        super();
    }

    setLogLevel(logLevel: string | IOTC_LOGGING) {
        if (typeof logLevel === 'string') {
            this.logLevel = IOTC_LOGGING[logLevel.toUpperCase() as keyof typeof IOTC_LOGGING];
            if (!this.logLevel) {
    ...
    async debug(message: string, tag?: string) {
        if (this.logLevel === IOTC_LOGGING.ALL) {
            ...
            this.emit(this.eventName, {
                eventName: `DEBUG${tag ? ` - ${tag.toUpperCase()}` : ''}`,
                eventData: message
            });
        }
    }
    async log(message: string, tag?: string) {
        if (this.logLevel != IOTC_LOGGING.DISABLED)
            this.emit(this.eventName, {
                eventName: `INFO${tag ? ` - ${tag.toUpperCase()}` : ''}`,
                eventData: message
            });
