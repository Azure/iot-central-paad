...
import {
  ...
  NavigationParams, DATA_AVAILABLE_EVENT
} from './types';
...
const Insight = React.memo<{ route: RouteProp<Record<string, NavigationParams & { chartType: ChartType, telemetryId: string; currentValue: any }>, 'Insight'> }>(({ route }) => {
    ...
    (id: string, value: any) => {
      ...
      itemToProcess.forEach((itemdata) => {
        setData((currentDataSet) => {
          ...
          if (currentItemData.values?.[currentItemData.values?.length] === itemdata.value) {
  ...
  useEffect(() => {
    ...
    return () => { sensor?.removeListener(DATA_AVAILABLE_EVENT, updateData) }
